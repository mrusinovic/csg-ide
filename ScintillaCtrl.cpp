...
struct loader{
	...
	loader(){
		...
		szFullPath[0] = _T('\0');
		if (GetModuleFileName(NULL, szFullPath, _countof(szFullPath))){
			...
			szExt[0] = _T('\0');
			_tsplitpath_s(L"SciLexer.dll", NULL, 0, NULL, 0, szFname, sizeof(szFname)/sizeof(TCHAR), szExt, sizeof(szExt)/sizeof(TCHAR));
			_tmakepath_s(szFullPath, sizeof(szFullPath)/sizeof(TCHAR), szDrive, szDir, szFname, szExt);

			//Delegate to LoadLibrary    
			hSci = LoadLibrary(szFullPath);
		}
	}
	~loader(){
		if (hSci) FreeLibrary(hSci);
	}
};
